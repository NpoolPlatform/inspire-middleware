...
func (gau *GoodAchievementUpdate) defaults() error {
	if _, ok := gau.mutation.UpdatedAt(); !ok {
		if goodachievement.UpdateDefaultUpdatedAt == nil {
			return fmt.Errorf("ent: uninitialized goodachievement.UpdateDefaultUpdatedAt (forgotten import ent/runtime?)")
		}
...
// defaults sets the default values of the builder before save.
func (gauo *GoodAchievementUpdateOne) defaults() error {
...
func (gauo *GoodAchievementUpdateOne) sqlSave(ctx context.Context) (_node *GoodAchievement, err error) {
	...
	if !ok {
		return nil, &ValidationError{Name: "id", err: errors.New(`ent: missing "GoodAchievement.id" for update`)}
	}
	...
	_spec.Modifiers = gauo.modifiers

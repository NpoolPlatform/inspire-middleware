...
func (*OrderPaymentStatement) scanValues(columns []string) ([]interface{}, error) {
	...
	for i := range columns {
		...
		default:
			return nil, fmt.Errorf("unexpected column %q for type OrderPaymentStatement", columns[i])
		}
	}
	return values, nil
}
...
func (ops *OrderPaymentStatement) assignValues(columns []string, values []interface{}) error {
...
func (ops *OrderPaymentStatement) Update() *OrderPaymentStatementUpdateOne {
	return (&OrderPaymentStatementClient{config: ops.config}).UpdateOne(ops)
}
...
func (ops *OrderPaymentStatement) Unwrap() *OrderPaymentStatement {
	...
	if !ok {
		panic("ent: OrderPaymentStatement is not a transactional entity")
	}
...
func (ops OrderPaymentStatements) config(cfg config) {
	for _i := range ops {
		ops[_i].config = cfg
	}
}

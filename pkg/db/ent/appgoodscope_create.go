...
func (agsc *AppGoodScopeCreate) Save(ctx context.Context) (*AppGoodScope, error) {
	...
	if len(agsc.hooks) == 0 {
		...
		var mut Mutator = MutateFunc(func(ctx context.Context, m Mutation) (Value, error) {
			...
			agsc.mutation = mutation
			if node, err = agsc.sqlSave(ctx); err != nil {
				return nil, err
			}
			mutation.id = &node.ID
			mutation.done = true
		...
		for i := len(agsc.hooks) - 1; i >= 0; i-- {
			if agsc.hooks[i] == nil {
				return nil, fmt.Errorf("ent: uninitialized hook (forgotten import ent/runtime?)")
			}
			mut = agsc.hooks[i](mut)
		}
		v, err := mut.Mutate(ctx, agsc.mutation)
		if err != nil {
			return nil, err
		}
...
func (agsc *AppGoodScopeCreate) defaults() error {
	if _, ok := agsc.mutation.CreatedAt(); !ok {
		if appgoodscope.DefaultCreatedAt == nil {
			return fmt.Errorf("ent: uninitialized appgoodscope.DefaultCreatedAt (forgotten import ent/runtime?)")
		}
		v := appgoodscope.DefaultCreatedAt()
		agsc.mutation.SetCreatedAt(v)
	}
	if _, ok := agsc.mutation.UpdatedAt(); !ok {
		if appgoodscope.DefaultUpdatedAt == nil {
			return fmt.Errorf("ent: uninitialized appgoodscope.DefaultUpdatedAt (forgotten import ent/runtime?)")
		}
		v := appgoodscope.DefaultUpdatedAt()
		agsc.mutation.SetUpdatedAt(v)
	}
	if _, ok := agsc.mutation.DeletedAt(); !ok {
		if appgoodscope.DefaultDeletedAt == nil {
			return fmt.Errorf("ent: uninitialized appgoodscope.DefaultDeletedAt (forgotten import ent/runtime?)")
		}
	...
	if _, ok := agsc.mutation.CouponID(); !ok {
		if appgoodscope.DefaultCouponID == nil {
			return fmt.Errorf("ent: uninitialized appgoodscope.DefaultCouponID (forgotten import ent/runtime?)")
		}
...
func (agsc *AppGoodScopeCreate) check() error {
	if _, ok := agsc.mutation.CreatedAt(); !ok {
		return &ValidationError{Name: "created_at", err: errors.New(`ent: missing required field "AppGoodScope.created_at"`)}
	}
	if _, ok := agsc.mutation.UpdatedAt(); !ok {
		return &ValidationError{Name: "updated_at", err: errors.New(`ent: missing required field "AppGoodScope.updated_at"`)}
	}
	if _, ok := agsc.mutation.DeletedAt(); !ok {
...
func (u *AppGoodScopeUpsertOne) Exec(ctx context.Context) error {
	if len(u.create.conflict) == 0 {
		return errors.New("ent: missing options for AppGoodScopeCreate.OnConflict")
	}
...
func (u *AppGoodScopeUpsertBulk) Exec(ctx context.Context) error {
	for i, b := range u.create.builders {
		if len(b.conflict) != 0 {
			return fmt.Errorf("ent: OnConflict was set for builder %d. Set it on the AppGoodScopeCreateBulk instead", i)
		}
	}
	if len(u.create.conflict) == 0 {
		return errors.New("ent: missing options for AppGoodScopeCreateBulk.OnConflict")
	}
	return u.create.Exec(ctx)
}

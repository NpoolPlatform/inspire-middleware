...
func (*TaskConfig) scanValues(columns []string) ([]interface{}, error) {
	...
	for i := range columns {
		...
		default:
			return nil, fmt.Errorf("unexpected column %q for type TaskConfig", columns[i])
		}
	}
	return values, nil
}
...
func (tc *TaskConfig) assignValues(columns []string, values []interface{}) error {
...
func (tc *TaskConfig) Update() *TaskConfigUpdateOne {
	return (&TaskConfigClient{config: tc.config}).UpdateOne(tc)
}
...
func (tc *TaskConfig) Unwrap() *TaskConfig {
	...
	if !ok {
		panic("ent: TaskConfig is not a transactional entity")
	}
...
func (tc TaskConfigs) config(cfg config) {
	for _i := range tc {
		tc[_i].config = cfg
	}
}

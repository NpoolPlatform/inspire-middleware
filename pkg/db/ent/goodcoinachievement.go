...
func (*GoodCoinAchievement) scanValues(columns []string) ([]interface{}, error) {
	...
	for i := range columns {
		...
		default:
			return nil, fmt.Errorf("unexpected column %q for type GoodCoinAchievement", columns[i])
		}
	}
	return values, nil
}
...
func (gca *GoodCoinAchievement) assignValues(columns []string, values []interface{}) error {
...
func (gca *GoodCoinAchievement) Update() *GoodCoinAchievementUpdateOne {
	return (&GoodCoinAchievementClient{config: gca.config}).UpdateOne(gca)
}
...
func (gca *GoodCoinAchievement) Unwrap() *GoodCoinAchievement {
	...
	if !ok {
		panic("ent: GoodCoinAchievement is not a transactional entity")
	}
...
func (gca GoodCoinAchievements) config(cfg config) {
	for _i := range gca {
		gca[_i].config = cfg
	}
}

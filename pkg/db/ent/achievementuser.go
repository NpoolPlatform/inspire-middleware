...
func (*AchievementUser) scanValues(columns []string) ([]interface{}, error) {
	...
	for i := range columns {
		...
		default:
			return nil, fmt.Errorf("unexpected column %q for type AchievementUser", columns[i])
		}
	}
	return values, nil
}
...
func (au *AchievementUser) assignValues(columns []string, values []interface{}) error {
...
func (au *AchievementUser) Update() *AchievementUserUpdateOne {
	return (&AchievementUserClient{config: au.config}).UpdateOne(au)
}
...
func (au *AchievementUser) Unwrap() *AchievementUser {
	...
	if !ok {
		panic("ent: AchievementUser is not a transactional entity")
	}
...
func (au AchievementUsers) config(cfg config) {
	for _i := range au {
		au[_i].config = cfg
	}
}

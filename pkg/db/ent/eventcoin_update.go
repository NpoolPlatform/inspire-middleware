...
func (ecu *EventCoinUpdate) defaults() error {
	if _, ok := ecu.mutation.UpdatedAt(); !ok {
		if eventcoin.UpdateDefaultUpdatedAt == nil {
			return fmt.Errorf("ent: uninitialized eventcoin.UpdateDefaultUpdatedAt (forgotten import ent/runtime?)")
		}
...
// defaults sets the default values of the builder before save.
func (ecuo *EventCoinUpdateOne) defaults() error {
...
func (ecuo *EventCoinUpdateOne) sqlSave(ctx context.Context) (_node *EventCoin, err error) {
	...
	if !ok {
		return nil, &ValidationError{Name: "id", err: errors.New(`ent: missing "EventCoin.id" for update`)}
	}

...
func (h *createHandler) createOrAddArchivement(ctx context.Context, tx *ent.Tx, req *statementcrud.Req) error {
	...
	stm, err := achivementcrud.SetQueryConds(
		tx.Achivement.Query(),
		&achivementcrud.Conds{
	...
	if info == nil {
		if _, err = achivementcrud.CreateSet(
			tx.Achivement.Create(),
			_req,
		).Save(ctx); err != nil {
			return err
		}
	...
	if _, err := achivementcrud.UpdateSet(
		tx.Achivement.UpdateOneID(info.ID),
		_req,
	).Save(ctx); err != nil {
		return err
	}

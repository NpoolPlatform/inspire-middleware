...
func (h *createHandler) constructSQL() {
	...
	_sql := "insert into app_good_commission_configs "
	_sql += "("
	if h.EntID != nil {
		_sql += "ent_id"
		comma = ", "
	...
	_sql += " and not exists ("
	_sql += " select 1 from app_good_commission_configs "
	_sql += fmt.Sprintf("where app_id='%v' and app_good_id='%v' and settle_type='%v' and level=%v and end_at=0 and deleted_at=0 and %v >= %v",
		*h.AppID, *h.AppGoodID, h.SettleType.String(), *h.Level, *h.StartAt, now)
	_sql += " limit 1)"
	h.sql = _sql
}
...
func (h *createHandler) createCommissionConfig(ctx context.Context, tx *ent.Tx) error {
	...
	if err != nil || n != 1 {
		return wlog.Errorf("fail create appgoodcommissionconfig: %v", err)
	}
	return nil
}

...
type Handler struct {
	...
	Conds  *invitationcodecrud.Conds
	Offset int32
	Limit  int32
}
...
func WithInvitationCode(code *string, must bool) func(context.Context, *Handler) error {
	return func(ctx context.Context, h *Handler) error {
		if code == nil {
			if must {
				return wlog.Errorf("invalid code")
			}
			return nil
		}
		if len(*code) != codegenerator.InvitationCodeLen {
			return wlog.Errorf("invalid invitationcode")
		}
...
func WithConds(conds *npool.Conds) func(context.Context, *Handler) error {
	return func(ctx context.Context, h *Handler) error {
		...
		if conds.InvitationCode != nil {
			h.Conds.InvitationCode = &cruder.Cond{
				Op:  conds.GetInvitationCode().GetOp(),
				Val: conds.GetInvitationCode().GetValue(),
			}
		}

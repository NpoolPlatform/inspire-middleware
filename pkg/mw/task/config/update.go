...
func (h *updateHandler) constructSQL() error {
	...
	_sql += fmt.Sprintf("where di.app_id = '%v' and di.event_id = '%v' and di.task_type = '%v' and di.id != %v and deleted=0", h.appID, *h.EventID, h.TaskType.String(), *h.ID)
	_sql += " limit 1)"

	if h.EventID != nil {
		_sql += "and not exists ("
		_sql += "select 1 from (select * from task_users) as di "
		_sql += fmt.Sprintf("where di.app_id = '%v' and di.task_id = '%v' and di.event_id = '%v' and deleted=0", h.appID, *h.EntID, h.eventID)
		_sql += " limit 1)"
	}

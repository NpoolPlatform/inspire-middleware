...
func updateCoinAllocated(t *testing.T) {
	err := UpdateCoinAllocated(context.Background(), &npool.CoinAllocatedReq{
		ID:    &ret.ID,
		EntID: &ret.EntID,
		Value: &ret.Value,
	})
...
func TestCoinAllocated(t *testing.T) {
	...
	t.Run("updateCoinAllocated", updateCoinAllocated)

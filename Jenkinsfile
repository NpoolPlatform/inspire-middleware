  ...
  stages {
    ...
    stage('Config target') {
      ...
      steps {
        ...
        sh(returnStdout: false, script: '''
          ...
          kubectl exec --namespace kube-system mysql-0 -- mysql -h 127.0.0.1 -uroot -p$PASSWORD -P3306 -e "create database if not exists inspire_manager;"
          ...
          for vhost in `cat cmd/*/*.viper.yaml | grep hostname | awk '{print $2}' | sed 's/"//g' | sed 's/\\./-/g'`; do
            ...
            ./apollo-item-config.sh $APP_ID $TARGET_ENV $vhost database_name inspire_manager
            cd -
          done
        '''.stripIndent())
      }
    ...
    stage('Release docker image for feature') {
      ...
      steps {
        sh(returnStdout: false, script: '''
          feature_name=`echo $BRANCH_NAME | awk -F '/' '{ print $2 }'`
          set +e
          docker images | grep inspire-middleware | grep $feature_name
          rc=$?
          set -e
          if [ 0 -eq $rc ]; then
            TAG=$feature_name DOCKER_REGISTRY=$DOCKER_REGISTRY make release-docker-images
          fi
          images=`docker images | grep entropypool | grep inspire-middleware | grep none | awk '{ print $3 }'`
    ...
    stage('Release docker image for development') {
      ...
      steps {
        sh(returnStdout: false, script: '''
          set +e
          docker images | grep inspire-middleware | grep latest
          rc=$?
          set -e
          if [ 0 -eq $rc ]; then
      ...
      steps {
        sh(returnStdout: false, script: '''
          ...
          if [ 0 -eq $rc -a x"$revlist" != x ]; then
            tag=`git describe --tags $revlist`
            set +e
            docker images | grep inspire-middleware | grep $tag
            rc=$?
            set -e
            if [ 0 -eq $rc ]; then
    ...
    stage('Deploy for feature') {
      ...
      steps {
        sh(returnStdout: false, script: '''
          feature_name=`echo $BRANCH_NAME | awk -F '/' '{ print $2 }'`
          sed -i "s/inspire-middleware:latest/inspire-middleware:$feature_name/g" cmd/inspire-middleware/k8s/02-inspire-middleware.yaml
          sed -i "s/uhub.service.ucloud.cn/$DOCKER_REGISTRY/g" cmd/inspire-middleware/k8s/02-inspire-middleware.yaml
    ...
    stage('Deploy for development') {
      ...
      steps {
        sh 'sed -i "s/uhub.service.ucloud.cn/$DOCKER_REGISTRY/g" cmd/inspire-middleware/k8s/02-inspire-middleware.yaml'
    ...
    stage('Deploy for testing') {
      ...
      steps {
        sh(returnStdout: false, script: '''
          ...
          git reset --hard
          git checkout $tag
          sed -i "s/inspire-middleware:latest/inspire-middleware:$tag/g" cmd/inspire-middleware/k8s/02-inspire-middleware.yaml
          sed -i "s/uhub.service.ucloud.cn/$DOCKER_REGISTRY/g" cmd/inspire-middleware/k8s/02-inspire-middleware.yaml

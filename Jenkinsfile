    ...
    stage('Release docker image for development') {
      ...
      steps {
        sh(returnStdout: false, script: '''
          ...
          set +e
          docker images | grep inspire-middleware | grep $branch
          rc=$?
          set -e
          if [ 0 -eq $rc ]; then
    ...
    stage('Deploy for development') {
      ...
      steps {
        sh(returnStdout: false, script: '''
          ...
          sed -i "s/inspire-middleware:latest/inspire-middleware:$branch/g" cmd/inspire-middleware/k8s/02-inspire-middleware.yaml
